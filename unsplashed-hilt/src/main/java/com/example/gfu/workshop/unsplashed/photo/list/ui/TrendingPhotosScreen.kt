...
@Composable
fun TrendingPhotosScreen(
    viewModel: TrendingPhotosViewModel,
    onNavigateToUser: (username: String) -> Unit = {}
) {
    val state = viewModel.uiState.collectAsState().value
    val sideEffects = viewModel.oneTimeActions


    val context = LocalContext.current

    LaunchedEffect(sideEffects, onNavigateToUser, context) {
        sideEffects.collect {
            when (it) {
                is TrendingSideEffect.NavigateToUser -> onNavigateToUser(it.userId)
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            items(state.photos) {
                TrendingPhotoCard(
                    photo = it,
                    onClick = { eventPublisher(TrendingEvent.PhotoClicked(it)) }
                )
            }
            if (state.loading) {
                item {
